...
function CheckoutForm() {
  ...
  function formatExpiryDate(value: string) {
    const valueDigits = onlyDigits(value).slice(0, 4)
  ...
  async function submitPlan() {
    ...
    try {
      ...
      const accountResponse = await fetch(`/api/sessions/${sessionId}/steps`, {
        ...
        body: JSON.stringify({ step: 12, data: { email } }),
      })
    ...
    <section className="max-w-lg mx-auto px-4 pb-12">
      ...
      {!isFreeTrial && <div className="mt-6 rounded-2xl border border-slate-700 bg-slate-800 p-5">
        ...
        <div className="mt-4 space-y-3">
          ...
          <input value={cardNumber} onChange={(event) => setCardNumber(formatCardNumber(event.target.value))} inputMode="numeric" placeholder="Card number (e.g. 4242 4242 4242 4242)" className="checkout-input" autoComplete="cc-number" />
          <div className="grid grid-cols-2 gap-3"><input value={expiry} onChange={(event) => setExpiry(formatExpiryDate(event.target.value))} inputMode="numeric" placeholder="MM/YY" className="checkout-input" autoComplete="cc-exp" /><input value={cvc} onChange={(event) => setCvc(onlyDigits(event.target.value).slice(0, 4))} inputMode="numeric" placeholder="CVC" className="checkout-input" autoComplete="cc-csc" /></div>
        </div>
      </div>}

      <button disabled={!canPay || processing} onClick={submitPlan} className="mt-5 w-full rounded-2xl bg-orange-500 py-4 font-bold shadow-lg shadow-orange-500/20 transition hover:bg-orange-600 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-500">
...
export default function SubscribePage() {
  return <Suspense fallback={<div className="min-h-screen bg-slate-900" />}><CheckoutForm /></Suspense>
}
